...
public class CommitButtonExt implements IButtonActionExt {
    ...
    private void handleCrAction(BtnActionBo btnActionBo, BtnCallResultBo resultBo, String bizKey) {
        ...
        if (isSignOffNode(btnActionBo)) {
            ...
            handleSignOffNodeAction(btnActionBo, resultBo, bizKey, formData);
        }
    ...
    private void handleCloseNodeAction(BtnActionBo btnActionBo, BtnCallResultBo resultBo, String bizKey, JsonNode formData) {
        ...
        if (crStatusValue.equals("Closed Successful")) {
            ...
            JSONObject ticketResult = getApiTenByParam("", paramBody);
    ...
    private void buildRcaMdl(MdlInstance racMdl, ObjectNode formDataJson, ProcessInfoPo rcaProcessInfo, MdlInstance crWorkOrderMdl, String rcaProcessDefId) {
        ...
        racMdl.setTopAccountId(UserHolder.get().getTopAccountId());

        racMdl.setAccountId(UserHolder.get().getAccountId());

        racMdl.setIsTest(0L);

        racMdl.setCreatedBy(racMdl.getCreatedBy());
    ...
    public static JSONObject getApiTenByParam(String url, JSONObject body) {
    ...
    private void handleSignOffNodeAction(BtnActionBo btnActionBo, BtnCallResultBo resultBo, String bizKey, JsonNode formData) {
...
//                    btnActionBo.setFormData(formDataResult);
    ...
    private void handelDroneAutoApproval(BtnActionBo btnActionBo, BtnCallResultBo resultBo, String bizKey, JsonNode formData){
        ...
        JSONObject ticketResult = getApiTenByParam(droneUrl, paramBody);
        ...
        if("1".equals(readyForCRApprovalFlag)){
            JSONArray releaseTicketsJson = ticketResult.getJSONArray("releasetickets");
    ...
    private void calculateSignOffResult(String workOrderId) {
        ...
        try {
            ...
            String url = dosmCustomDomainName + "/dcs/dosm/signOff/calculateSignOffResult";
            String response = this.get(url + "?workOrderId=" + workOrderId, null);
            ...
            throw new BaseException("get signOff approval result error");
        }

        // 判断是否通过
        if (!result) {
            throw new BaseException("SignOff approval result is failed");
        }
    }

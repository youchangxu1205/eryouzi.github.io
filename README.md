...
public class CrStatusSyncTrigger implements ITriggerExtV2 {
        ...
        try {
            ...
            if ("200".equals(node.get("code").asText())) {
                ...
                if ("FAILED".equals(status.asText())) {
                    ...
                    saveApiLog(url, jsonString, body, workOrder.getBizKey(), "crStatusSync", false);
                } else {
                    saveApiLog(url, jsonString, body, workOrder.getBizKey(), "crStatusSync", true);
                }
            } else {
                ResultBean resultBean = ctx.getResult();
    ...
    public void makeFormInfo(ObjectNode formData, WorkOrderBean workOrder, ExtParam extParam) {
        ...
        JsonNode node = formData.get("codechecker");
        if(node != null && node.asText().startsWith("Yes")){
            formData.put("codechecker","Yes - Yes");
        }
